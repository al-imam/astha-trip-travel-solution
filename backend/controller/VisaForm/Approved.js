...
const approved = async (req,res)=>{
    try {
        ...
        console.log("🚀 ~ file: Approved.js:9 ~ approved ~ {id,country}:", {id,country})
        ...
        res.status(400).send('bad request `⛔ Eroor in Approved.js > path = backend>controller>VisaForm>Approved.js : line= 24 `')


    } catch (error) {
        console.log("🚀 ~ file: Approved.js:5 ~ approved ~ error:", error)
        res.status(500).send({error,hint: `⛔ Eroor in Approved.js > path = backend>controller>VisaForm>Approved.js : line= 29 `})
    }
}
