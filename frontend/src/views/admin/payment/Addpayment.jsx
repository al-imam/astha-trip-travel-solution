...
const Addpayment = ({close}) => {
  ...
  useEffect(() => {
    const Getdata = async () => {
      try {
        ...
        SetAllAgent(serverRes.data?.map((e) => {
  ...
  useEffect(() => {
    if (SelectedAgent) {
      const getAgentData = async () => {
        try {
          const serverRes = await toast.promise(axios.post('/api/loi/loibyagent', {
            email: SelectedAgent
          }), {
            pending: "please Wait ..",
  ...
  return (
    <div className='w-full fixed top-0 bg-brand-100/20 z-50 left-0 backdrop-blur-sm h-screen overflow-y-scroll'>
      <div className='w-full relative pt-3 px-5'>
        <div className='w-full relative p-2 bg-brand-400 rounded-t-md shadow-md'></div>
        <div className='w-full relative border-x-2 rounded-b-md border-brand-400 border-b-2 p-3 bg-white/80 backdrop-blur-2xl'>

          <div className='w-full relative'>
            <h1 className='text-xl font-bold flex justify-center items-center border-b-2 border-brand-100 pb-2'>
              <span className='text-3xl'><FluentMoneyHand24Filled /></span> Add Payment History
            </h1>
          </div>

          <div className='w-full relative grid grid-cols-2 gap-2'>
            <div className='w-full relative p-3 md:col-span-1 col-span-2'>
              <label className='pl-1 py-2 flex items-center'>
                Choose Agent:{
                  loading ? (<span className='relative pl-2 text-brand-600'><EosIconsBubbleLoading /></span>) : ""
                }
              </label>
              <CreatableSelect
                isDisabled={loading}
                ...
                value={{ label: SelectedAgent, value: SelectedAgent }}
                options={allAgent}
                styles={{
          ...
          {
            FetchAllLOI.length ? (<div>
              <div className='w-full relative flex justify-between gap-3 py-3 xl:flex-nowrap flex-wrap'>
                <Widget
              ...
              <div className='w-full h-72 flex justify-center items-center flex-col'>
                <div className='text-5xl text-brand-500'><LineMdBeerLoop /></div>
                <p className='text-xl font-extralight tracking-wide pt-3'>No Data Available Please Select Agent Or Another Agent</p>
              </div>
            </div>)
          ...
          <dir className="w-full relative p-3">
            <button 
            onClick={()=>{close(false)}}
            className='bg-red-500 px-5 py-2 text-white rounded-md '>Close</button>
          </dir>
        </div>
      </div>
    </div >
...
export function MaterialSymbolsDonutSmallOutline(props) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" {...props}><path fill="currentColor" d="M13.025 2.05q3.575.35 6.088 2.863T21.974 11h-7.15q-.225-.65-.687-1.137t-1.113-.713v-7.1Zm2 2.55V8q.275.225.525.475t.475.525h3.4q-.6-1.5-1.75-2.65t-2.65-1.75Zm-4-2.55v7.1q-.9.325-1.45 1.113T9.025 12q0 .95.55 1.713t1.45 1.087v7.15q-3.85-.375-6.425-3.225T2.025 12q0-3.875 2.575-6.725t6.425-3.225Zm-2 2.55q-2.275.875-3.638 2.9T4.026 12q0 2.475 1.363 4.5t3.637 2.95V16q-.95-.725-1.475-1.763T7.025 12q0-1.2.525-2.238T9.025 8V4.6Zm5.8 8.4h7.15q-.35 3.575-2.863 6.088t-6.087 2.862V14.8q.65-.225 1.113-.688T14.825 13Zm1.2 2q-.2.275-.462.525t-.538.475v3.4q1.5-.6 2.65-1.75t1.75-2.65h-3.4Zm-9-2.975Zm9-3.025Zm0 6Z"></path></svg>
  )
}
...
export function LineMdBeerLoop(props) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" {...props}><mask id="lineMdBeerLoop0"><path stroke="#fff" strokeWidth="2" d="M18 7C16 7 15 9 13 9C11 9 10 7 8 7C6 7 5 9 3 9C1 9 0 7 -2 7C-4 7 -5 9 -7 9" opacity="0"><animateMotion calcMode="linear" dur="3s" path="M0 0h10" repeatCount="indefinite"></animateMotion><animate fill="freeze" attributeName="opacity" begin="0.6s" dur="0.5s" values="0;1"></animate></path></mask><path fill="none" stroke="currentColor" strokeDasharray="60" strokeDashoffset="60" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M18 3L16 21H7L5 3z"><animate fill="freeze" attributeName="stroke-dashoffset" dur="0.6s" values="60;0"></animate></path><path fill="currentColor" d="M18 3L16 21H7L5 3z" mask="url(#lineMdBeerLoop0)"></path></svg>
  )
}